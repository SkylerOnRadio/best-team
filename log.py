...
def calculate_entropy(data: str) -> float:
    ...
    if not data or len(data) < 10:
        return 0.0
    # Strip leading timestamp/host tokens to reduce noise
...
def log_template(line: str) -> str:
    ...
    return t[:120]  # cap length for dict key sanity
...
def _risk_score(gaps: list, threats: list) -> int:
    ...
    for t in threats:
        ...
        # Kill-chain escalates probabilities
        ...
        if "KILL_CHAIN_DETECTED" in tags:
            # Raise all active zones by 10% on confirmed kill chain
            for z in zone_probs:
                if zone_probs[z] > 0:
...
def scan_log(filepath: str, threshold_seconds: float,
    ...
    Performs: timestamp parsing, integrity checking, IP profiling,
    signature matching, entropy detection, distributed attack detection,
    rare template detection, session reconstruction.
    """
    ...
    # For distributed attack detection: bucket → list of (ip, is_fail)
            ...
            for line_no, line in enumerate(fh, start=1):
                ...
                if prev_ts is not None:
                    ...
                    if diff >= threshold_seconds:
                        ...
                        # Only flag reversals > 10s to ignore multi-thread jitter
                        gaps.append({
                            "type": "REVERSED",
                            "gap_start": prev_ts.isoformat(),
                            "gap_end": ts.isoformat(),
                ...
                if ip_match:
                    ...
                    if ip not in ip_stats:
                        ip_stats[ip] = {
                            ...
                            "events": [],       # for session reconstruction
                            "tags": set(),
                        }
                    ...
                    # Entropy check with dynamic threshold
                    ent = calculate_entropy(line_content)
                    if ent > entropy_threshold:
                        stats["tags"].add("HIGH_ENTROPY_PAYLOAD")
                        obfuscated_count += 1

                    # Populate time bucket for distributed detection
    ...
    # Rare template flagging
    rare_templates = {t for t, c in template_counts.items() if c <= RARE_TEMPLATE_THRESHOLD}

    # Distributed attack detection: flag windows with many IPs failing
    ...
    # Build final threat list with enrichments
    final_threats = []
    ...
    # Comparative profile (optional second file)
    compare_result = None
...
def _compare_profile(filepath2: str, baseline_ip_stats: Dict) -> Dict:
    ...
    indicating lateral movement or new actors since the baseline log.
    """
    new_ips = set()
    try:
...
def report_csv_integrity(result: dict):
    ...
    print("[*] Integrity CSV → report.csv")
...
def report_csv_behavioral(result: dict):
    ...
    print("[*] Behavioral CSV → target.csv")


def report_json(result: dict):
    with open("report.json", "w", encoding="utf-8") as f:
        json.dump(result, f, indent=2, default=str)
    print("[*] JSON export → report.json")
...
def report_html(result: dict, filepath: str):
    ...
    def gen_threat_rows(subset: list) -> str:
        ...
        for t in subset:
            ...
            tag_html_str = " ".join(tag_html(tg, "red" if tg in ("KILL_CHAIN_DETECTED","KNOWN_MALICIOUS_IOC","LOG_TAMPERING","DATA_EXFIL") else "blue")
    ...
    # Sparkline-style hit bars for top actors
    actor_bars = ""
    ...
    if result.get("compare") and result["compare"]["count"]:
        ...
        <div class="card">
            ...
            <p style="color:var(--secondary);font-size:13px;">
                ...
                These represent potentially new or previously unseen threat actors.
            </p>
...
<footer>Evidence Protector Engine v6.0 &nbsp;|&nbsp; {stats['parsed']:,} lines parsed &nbsp;|&nbsp; {stats['skipped']:,} noisy lines skipped &nbsp;|&nbsp; Generated {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</footer>
    ...
    print("[*] HTML dashboard → report.html")
...
Examples:
  ...
  %(prog)s system.log --format terminal --threshold 60
    ...
    fmt = args.format
    if fmt in ("all", "terminal"): report_terminal(result, args.logfile)
    if fmt in ("all", "csv"):      report_csv_integrity(result); report_csv_behavioral(result)
    if fmt in ("all", "html"):     report_html(result, args.logfile)
    if fmt in ("all", "json"):     report_json(result)
